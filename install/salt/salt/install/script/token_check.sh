...
for ((i=1;i<=60;i++ ));do
    ...
    [ ! -z "$notready" ] && (
        curl http://127.0.0.1:7999/login -H 'Accept: application/x-yaml' -d username=saltapi -d password=bie5Aiqueiwav5EeY6pae3ahLai3chee -d eauth=pam -s | grep token | awk '{print $2}' > /tmp/.token
    ) && break
done
) 
